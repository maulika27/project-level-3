...
function draw(){
	...
	/*	if(frog.isTouching(level1Sprites)){
		...
		fill("red");
		stroke("yellow");
		text("Score: "+level2Score,width-200,20);
...
function fireBallChallenge(){
 ...
 //var rand = Math.random(10 - 990)
		...
		fball.scale = 0.5;
		   ...
		   text("score: "+ level3Score, width-200, 100)

		   fball.collide(inviground3);

		   if(fball.isTouching(inviground3)){
			level3Score = level3Score+1;
			fball.destroy();
		   }

		   if(level3Score===3){
			   var dialogBox2 = createSprite(width/2, height/2);
